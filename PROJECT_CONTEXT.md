...
- Viewer mirror covers the device list, shell, and (as of v1.11.0/v1.15.0) Properties/Features/Packages/Attestation/RKP tabs for every device the host has visited this session (see `hostTabHtmlCache`, v1.15.0) — not just whichever one is currently active. A device the host has genuinely never selected at all still won't have anything cached.
