...
- **Device-switch guard**: `setMirrorActiveSerial(serial)` is the single place that reassigns `remoteSession.mirror.activeSerial` — both the manual click handler (`selectMirrorDevice()`) and `renderMirrorDeviceList()`'s own automatic fallback (when the mirrored serial disappears from the host's broadcast list) route through it, so switching to a different device while holding control always releases it first.
