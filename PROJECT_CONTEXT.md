...
### Screen/Input Remote Control (v1.14.0)
Vysor-style remote control: a viewer can request to see the host's device screen and send taps/swipes/text/hardware-key presses, which the host injects via `adb shell input ...`. Only one viewer can hold control at a time; others keep their own independent Remote Shell but are auto-denied if they request control while someone else holds it. Screen frames are sent only to the current controller, never broadcast.

- **Dispatch-dedup prerequisite fix**: before this feature, `makeRemoteActions()`'s dual-transport wrapper delivered every message via both the real P2P path and the fallback WS-relay path *independently* — when both transports were healthy (the common case), every message could be handled twice, under two different `ctx.peerId` values for the same sender (real trystero peerId vs. the fallback's per-tab session UUID). Harmless for old idempotent actions, but would have broken a stateful single-slot control system and made `inputEvent` (fire-and-forget, no natural idempotency) cause literal double-taps/double-swipes. Fixed by funneling both transports through one internal `deliver(name, data, ctx)` that dedups by `(action, requestId)` regardless of which transport won, and by setting `real.onMessage` exactly once at action creation (callers now only ever touch the `dispatchers` map, never `real.onMessage` directly — this also structurally rules out ever repeating the v1.10.x getter/setter-mismatch bug).
- **`grantId` + `seq` scoping**: `screenFrame` and `inputEvent` are high-frequency and not naturally request/response-shaped, so they're excluded from the requestId-based dedup above and instead carry their own ordering guard. `grantId` (a `crypto.randomUUID()` the host mints at grant time) scopes every message belonging to one control session end-to-end — both sides drop anything whose `grantId` doesn't match their current session, which fully eliminates cross-session bleed (e.g. a frame captured just before revoke arriving after a *new*, different grant has already started). `seq` (monotonic per grantId) additionally orders frames/input events within one session so a reordered delivery can't cause a visible rewind or a duplicate input action.
- **Dual-transport, not P2P-only**: `screenFrame`/`inputEvent` briefly skipped the fallback relay (see the "Relay-based fallback data path" note above) but were reverted to full dual-transport the same day after real-world testing showed a fallback-only viewer got a working Grant/Deny handshake but a permanently-broken screen — the exact failure mode this whole fallback channel exists to prevent. They now go over both transports like everything else; `grantId`+`seq` scoping is what makes that safe.
- **Binary-safe capture**: `adbScreencap(adb, timeoutMs)` mirrors `adbShellTimed()`'s `AbortController`+`process.kill()` pattern but pipes through `ConcatBufferStream` (not `TextDecoderStream`) to get raw PNG bytes — the existing `adbShell`/`adbShellTimed` are both text-decoded and would corrupt binary image data. `captureScaledFrame(adb, maxWidth, jpegQuality)` decodes that PNG via `createImageBitmap`, scales it down onto an `OffscreenCanvas` (target ~480px wide) and re-encodes as JPEG (`quality` ~0.55) before base64-encoding (`uint8ToBase64()`, chunked to avoid a stack overflow on `String.fromCharCode`) — this keeps each frame small enough to push over a P2P data channel several times a second. The real (pre-scale) device resolution is returned alongside so the viewer can map tap/swipe coordinates from the smaller transmitted image back to real device pixels.
- **State**: host adds `remoteSession.control` (`{peerId, serial, grantId, seq, lastInputSeq} | null`) and `remoteSession.pendingControlRequest` (`{peerId, serial, requestId} | null`, one at a time — a second concurrent request is auto-denied). Viewer adds `remoteSession.control` (`{active:true, serial, grantId, lastAppliedSeq, inputSeq, lastFrameW, lastFrameH} | null`).
- **Host flow**: `handleControlRequest()` → `showControlRequestPrompt()` (an overlay+box modal built at runtime, same pattern as `showShareModal()`) → `grantControlRequest()`/`denyControlRequest()`. Granting starts `startScreencapLoop()`, a recursive `setTimeout` (not `setInterval`, so a slow capture never overlaps the next one) at ~250ms intervals that re-checks `remoteSession.control`'s `grantId` on both sides of each async capture, so a revoke mid-capture just stops the loop rescheduling itself rather than sending a stale frame into a new session. `handleInputEvent()` dispatches by `type` to plain `adbShell()` calls (`input tap/swipe/keyevent/text`) — `toSafeInt()` coerces all numeric fields before they're interpolated into the shell command string, since these values are otherwise attacker-controllable-in-principle even from an already-granted session.
- **Cleanup**: `notifyDeviceRemoved(serial)` is called from all six independent `connectedDevices.delete(...)` call sites (`handleUsbError`, the USB-disconnect-event matcher, the `connection.closed` handler, the heartbeat-failure handler, `disconnectOne`, `disconnectDevice`) so a device going away always tears down any control session or pending request tied to it, regardless of which of the six paths triggered the removal. `handlePeerLeaveHost()` and `stopShareSession()` also clear control state on viewer-disconnect / host-stops-sharing.
- **Viewer UI**: a new "Remote Control" tab, reachable only by viewers. Per a CSS-specificity check done before implementing: `.viewer-only` must **not** be applied to a `.tab-content` element, since `#inspector-section.viewer-mode .viewer-only { display:block }` (ID + 2 classes) beats `.tab-content.active { display:block }` (2 classes) and would force the tab visible regardless of which tab is actually selected. Only the tab-bar **button** (`#tab-btn-control`) gets `.viewer-only`; the tab-content (`#tab-control`) is a completely ordinary tab, unreachable by a host simply because its button doesn't exist for them.
- **Control is pinned to its serial, not to `mirror.activeSerial`** (revised v1.15.0): `setMirrorActiveSerial(serial)` is the single place that reassigns `remoteSession.mirror.activeSerial` — both the manual click handler (`selectMirrorDevice()`) and `renderMirrorDeviceList()`'s own automatic fallback (when the mirrored serial disappears from the host's broadcast list) route through it. It deliberately does NOT touch an active control session: an earlier version auto-released control on any activeSerial change, but since the automatic fallback path fires on every `devicePush` whose snapshot doesn't currently contain the mirrored serial, that ended up silently dropping control from unrelated host-side device-list churn, not just an intentional device switch. Control now only ends via explicit Release/Stop, a `controlRevoked` from the host, or the session ending.
- **Frame pacing** (tuned v1.15.0): the loop's inter-frame `setTimeout` is 15ms (just enough to yield to the event loop), down from an original 250ms fixed gap that was pure dead time on top of the actual on-device capture cost — real fps is now paced almost entirely by however long `screencap -p` + local scale/encode take on the target device, not by an artificial floor.
