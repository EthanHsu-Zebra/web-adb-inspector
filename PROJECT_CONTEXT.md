...
### State
`remoteSession` (module-level, `src/index.js`) — either `null`, a host session (`{role:'host', room, roomId, password, trusted, viewers:Set, actions, pendingApprovals:Map}`), or a viewer session (`{role:'viewer', room, roomId, password, hostPeerId, actions, pendingRequests:Map, mirror:{activeSerial, connected[], available[]}}`). `actions` (both roles) is what `makeRemoteActions()` returns — see below, not just a thin wrapper over `room.makeAction()` anymore.
