...
Known gap, low priority: `remoteSession.viewers` (host-side, used only for the share modal's "Connected viewers" count) is cleaned up on `room.onPeerLeave` — a P2P-only event. A fallback-only viewer who closes their tab without sending `bye` won't be removed from that count. Doesn't affect functionality, only that one display.
