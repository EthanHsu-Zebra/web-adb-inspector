...
Lets a second user (viewer), anywhere, with just a browser, join a session a host creates and run adb shell commands against the host's connected device. Primarily peer-to-peer via WebRTC; signaling goes over a self-hosted relay we control (`relay-server/`, deployed on Render — see the debugging history below for why this replaced public Nostr relays). As of v1.10.0, the *actual session traffic* (not just signaling) also has a fallback path over that same relay, for networks where WebRTC's P2P/TURN mechanics are blocked outright (see "Relay-based fallback data path" below) — `makeRemoteActions()` sends every message over both paths transparently, so nothing above that layer needs to know which one actually delivered it.

### State
`remoteSession` (module-level, `src/index.js`) — either `null`, a host session (`{role:'host', room, roomId, password, viewers:Set, actions}`), or a viewer session (`{role:'viewer', room, roomId, password, hostPeerId, actions, pendingRequests:Map, mirror:{activeSerial, connected[], available[]}}`). `actions` (both roles) is what `makeRemoteActions()` returns — see below, not just a thin wrapper over `room.makeAction()` anymore. There is no approval/trust gate (removed in v1.13.0) — the share link itself is the authorization boundary.

### Relay-based fallback data path (v1.10.0)
`@trystero-p2p/ws-relay`'s server (`relay-server/node_modules/@trystero-p2p/ws-relay/dist/server.mjs`) is a generic topic-based pub/sub over plain WebSocket (`subscribe`/`unsubscribe`/`publish` to an arbitrary topic string) — nothing about it is WebRTC-signaling-specific. `createFallbackChannel(roomId, password, sessionId, label)` is a second, independent client speaking that same protocol on a room-scoped topic (`webadb-fallback-<roomId>`), carrying AES-GCM-encrypted (key derived from the room password) copies of the actual session messages. `makeRemoteActions(room, roomId, password, label)` wraps every action (`hello`/`devicePush`/`cmdRequest`/`cmdResponse`/`pathComplete`/`pathCompleteResult`/`bye`) so `send()` goes out over both the real P2P channel (best-effort — silently a no-op if trystero doesn't recognize the target, same as its own behavior for an inactive peer) and the fallback channel, and `onMessage()` fires for whichever one actually delivers a given message first (deduped by `action:requestId` for message types that carry one, to avoid e.g. double-executing a `cmdRequest`). Every caller elsewhere in the file is unaware this exists — they only ever see `{send, onMessage}` and a `ctx.peerId`, which is now either a real trystero peerId or (fallback-only) the sender's random session UUID.
