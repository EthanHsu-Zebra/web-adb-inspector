...
- **Root cause found (2026-08-04) for cross-network failures from a corporate network host**: `@trystero-p2p/nostr`'s default relay pool is ~47 mostly small/self-hosted relay domains, 5 picked at random per `joinRoom()` call. A host on a locked-down corporate network showed a browser console error connecting to one of them (`relay.nostrdice.com` — WebSocket connection failed), and its own Debug Console never logged a single `WebRTC peer joined`, even though two independent remote viewers on different outside networks *did* each log a successful peer join — with two different peer IDs. That means the two viewers found **each other** (trystero rooms are a full mesh) but never the host: the host's network was blocking Nostr relay signaling outright, most likely via category/reputation-based web filtering (these relay domains look exactly like the obscure/uncategorized traffic such filters target).
- Fix applied: `REMOTE_RELAY_URLS` pins `joinRoom()`'s `relayConfig` to 4 of the most established, long-running public Nostr relays (`relay.damus.io`, `nos.lol`, `relay.nostr.band`, `relay.snort.social`) with `redundancy` set to try all 4 rather than a random 5-of-47, plus `warnOnRelayFailure: true` so relay-level failures also surface in the browser console. Not yet confirmed whether this is sufficient on the specific corporate network that triggered this investigation — if even these well-known relay domains are blocked, the next step would be a self-hosted signaling relay on a widely-allowlisted PaaS domain (e.g. Cloudflare Workers) via `@trystero-p2p/ws-relay`, which is a bigger architecture change (a real, if serverless/managed, piece of infra we'd operate) worth discussing before implementing.
