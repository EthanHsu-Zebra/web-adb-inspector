...
- **`p2pOnly` actions**: `screenFrame` and `inputEvent` skip the fallback relay entirely (`p2pOnlyActions` Set inside `makeRemoteActions()`) — sending several-times-a-second binary frames over the shared self-hosted relay used by every session of this app would be a real noisy-neighbor cost, and a WS-relay round trip is already too slow/jittery to make remote control usable on a fallback-only network anyway. Accepted degradation: no remote screen control on Zscaler-style/WebRTC-blocked networks — the text-based Remote Shell still works there via the normal dual-transport path.
