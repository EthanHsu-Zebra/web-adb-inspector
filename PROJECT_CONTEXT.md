...
- **Cross-network debugging saga (2026-08-04) — resolved as rate-limiting, NOT a firewall block.** A host on a corporate network repeatedly showed zero `WebRTC peer joined` events while two independent remote viewers instead found *each other* (trystero rooms are a full mesh) — initially looking exactly like the host's network was blocking Nostr relay signaling. First hypothesis (corporate web filter blocking obscure relay domains) led to pinning `joinRoom()`'s `relayConfig` to 4 well-known relays (`relay.damus.io`, `nos.lol`, `relay.nostr.band`, `relay.snort.social`). That made things *worse*: a plain `wss://echo.websocket.org` test proved WebSocket itself was never blocked, and the browser console instead showed explicit `Trystero: relay failure from wss://relay.damus.io/ - rate-limited` — our own ~hour of repeated test cycles (host + 2 viewers per round, multiple rounds) had concentrated load onto just 4 relays and tripped their abuse protection. **Fix: removed the relay pinning entirely**, reverting to `@trystero-p2p/nostr`'s default behavior (picks 5 random relays from its ~47-relay pool per `joinRoom()` call) — spreading load is the point of that design, overriding it to a small fixed set was the actual mistake, not the network.
- Lesson for future debugging here: a "stuck connecting" / "0 peers joined" symptom on this app is NOT reliable evidence of a firewall block by itself — check the browser console for explicit relay-level messages (e.g. `rate-limited`) before assuming network-level blocking, and avoid rapid repeated `joinRoom()` test cycles against the same relay set.
