...
- Viewer mirror covers the device list, shell, and (as of v1.11.0/v1.15.0) Properties/Features/Packages/Attestation/RKP tabs for every device the host has visited this session (see `hostTabHtmlCache`, v1.15.0) — not just whichever one is currently active. As of v1.17.0, Properties/Features/Packages specifically are also proactively fetched (`prefetchTabsForViewers()`) for every connected device the host *hasn't* personally clicked, while a share session is active — so only Attestation/RKP still require an actual visit (host or viewer-prompted) before a viewer can see them for a given device. As of v1.18.0, `startShareSession()` also explicitly re-broadcasts `activeSerial`'s own already-rendered tabs (all five) the moment sharing starts — needed because `pushTabHtml()` silently no-ops whenever `remoteSession` doesn't exist yet, which it never does at the moment a device auto-selects itself on connect, before the host has clicked Share.
