...
- **1.1.24** — Heartbeat: switched from `adb.getProp` (hangs on USB drop) to `usbDevice.opened` check — WebUSB API flips this to `false` instantly on physical cable removal. Connect Device: custom picker from `navigator.usb.getDevices()` filters out already-connected devices; falls back to native picker for first-time grant. Bumped APP_VERSION 1.1.23 -> 1.1.24.
