...
- **1.1.15** — Probe rendering: skip empty/null values in renderKV (verified_boot, security_hw empty props hidden). HW Trust: show clean "Identity service not available" status instead of raw error. Multi-device: USB disconnect auto-detection — device card updates to "Disconnected", auto-switches to next device. Switching devices via left panel now clears all tabs (Shell, HW Trust, RKP, Probe, Attestation) and refreshes all data for the new device. Disconnecting the active device auto-picks the next available device.
